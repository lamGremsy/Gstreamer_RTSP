...
media_configure (GstRTSPMediaFactory * factory, GstRTSPMedia * media, gpointer user_data)
{
    ...
    ctx->generator_pipe = gst_parse_launch ("v4l2src device=/dev/video0 ! video/x-raw, framerate=30/1, width=640, height=480 \
                                            ! videobox border-alpha=0 top=-2 bottom=-2 left=-2 right=-2 ! videomixer name=mix \
                                            ! x264enc speed-preset=superfast tune=4 ! appsink name=vid drop=true \
                                            v4l2src device=/dev/video2 ! video/x-raw, framerate=30/1, width=320, height=240 \
                                            ! videoconvert ! mix.", NULL);
